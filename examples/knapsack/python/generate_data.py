...
if __name__ == '__main__':
    ...
    weights_df = pd.DataFrame(weights.T, columns=['width', 'heigth'])
    x_df = pd.DataFrame(x, columns=[f'feat_{i}' for i in range(1, p+1)])
    c_df = pd.DataFrame(c, columns=[f'item_{i}' for i in range(1, m+1)])

    # store in csv files
    weights_df.to_csv(os.path.join(INPT_PATH, 'weights.csv'), index=False)
