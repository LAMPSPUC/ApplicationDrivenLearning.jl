...
if N_HIDDEN_LAYERS == 0
    nns = [
        Flux.Chain(Flux.Dense(lags*pd.n_demand => pd.n_demand)),
        Flux.Chain(Flux.Dense(1 => 2*pd.n_zones; bias=false),),
    ]
else
    nns = [
        Flux.Chain(
            Flux.Dense(lags*pd.n_demand => HIDDEN_SIZE), Flux.relu,
            [Flux.Dense(HIDDEN_SIZE => HIDDEN_SIZE, Flux.relu) for _=1:N_HIDDEN_LAYERS-1]...,
            Flux.Dense(HIDDEN_SIZE => pd.n_demand),
        ),
        Flux.Chain(Flux.Dense(1 => 2*pd.n_zones; bias=false))
    ]
end
input_output_map = [
    Dict(collect(1:lags*pd.n_demand) => collect(1:pd.n_demand)),
...
if pretrain
    ...
    while (epoch <= PRETRAIN_EPOCHS)
        Flux.train!(nns, train_data, opt_state) do m, x, y
            (m[1](x[1:end-1, :]) - y[1:pd.n_demand, :]).^2 |> sum
        end
        
        err2 = mean(
            sum(
                (nns[1](X_train[:, 1:end-1]')' - Y_train[:, 1:pd.n_demand]).^2, dims=2
            )
        )
