...
@testset "Multi-Variate-Dense" begin
    ...
    @test Flux.params(forecaster.networks[1])[1] == 0.8 * ones((model_out_size, model_in_size))
...
@testset "Multi-Model-Dense" begin
    ...
    forecaster = ApplicationDrivenLearning.PredictiveModel(
        [nn1, nn2], in_out_map, in_size, out_size
    )
    ...
    @test Flux.params(forecaster.networks[1])[1] == 0.9 * ones((model_out_size, model_in_size))
    @test Flux.params(forecaster.networks[1])[2] == 0.9 * ones(model_out_size)
    @test Flux.params(forecaster.networks[2])[1] == 0.9 * ones((model_out_size, model_in_size))
